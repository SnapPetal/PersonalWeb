(function () {
  ...
  // Upload - converts on server, then plays converted MP4
  videoUpload.addEventListener("change", async function (e) {
    ...
    frameCounter.textContent = "Uploading and converting...";
    ...
    try {
      ...
      console.log("Got new videoId:", currentVideoId);

      // Load converted video from server (with cache-busting)
      const videoUrl =
        "/skatetricks/video/" + currentVideoId + "?t=" + Date.now();
